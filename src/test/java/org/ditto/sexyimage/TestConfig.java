...
public class TestConfig {
    ...
    public ServerInterceptor globalInterceptor() {
        ...
        when(mock.interceptCall(
                notNull(ServerCall.class),
                notNull(Metadata.class),
                notNull(ServerCallHandler.class))
        ).thenAnswer(
                new Answer<ServerCall.Listener>() {
                    @Override
                    public ServerCall.Listener answer(InvocationOnMock invocation) throws Throwable {
                        return invocation
                                .getArgumentAt(2, ServerCallHandler.class)
                                .startCall(
                                        invocation.getArgumentAt(0, ServerCall.class),
